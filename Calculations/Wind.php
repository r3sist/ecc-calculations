...
//            $blc->table($flat);
                $scheme = ['$c\ (c_p, c_i)$', '$w [(kN)/m^2]$<!--success-->', 'Zóna szélesség $[m]$', 'Zóna mélység $[m]$', 'name'];

                $rows = [];
                $rows[0] = [];
                $i = 0;
                foreach ($flat as $key0 => $val0) {
                    $j = 0;
...
//            $blc->table($canopy,'Pilletető '.$case['wind'], '');
        ...
        $blc->boo('calcAttic', ['', 'Szabadon lló fal és mellvéd számítása'], false);
