...
Class Ck
{
    public function calc(Base $f3, Blc $blc, Ec $ec): void
    {
        ...
        $blc->def('Iyck', ceil(($f3->_bck* ($f3->_hck ** 3))/12), 'I_y = (b*h^3)/12 = %% [mm^4]', 'Inercia');
        $blc->def('Wck', ceil(($f3->_bck* ($f3->_hck ** 2))/6), 'W = (b*h^2)/6 = %% [mm^3]', 'Keresztmetszeti modulus');
        ...
        $blc->def('MEdck', H3::n3(($f3->_qd* (($f3->_Lck / 1000) ** 2))/8), 'M_(Ed) = (q_d*L^2)/8 = %% [kNm]', 'Nyomatéki igénybevétel');
