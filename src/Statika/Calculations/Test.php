...
Class Test
{
    ...
    public function calc(EurocodeInterface $ec): void
    {
        ...
        $ec->boo('b3', ['', 'boo2'], true, 'default `true` without math title');
        $ec->hr();

        $ec->h1('***region0/1*** tests');
        $ec->region0('r1', '*md* title');
            $ec->txt('blabla');
        $ec->region1();
        $ec->region0('r2');
            $ec->txt('without defined title');
        $ec->region1();
        $ec->region0('2 nem jó név');
        $ec->txt('with wrong name & without defined title');
        $ec->region1();
        ...
        $ec->h1('Normal h1');
        $ec->h1('***h1*** test $math$', 'md *subtitle* $math$');
